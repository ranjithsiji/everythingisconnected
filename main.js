  ...
  */
    ...
    var createBoard = function (lang, board) {
      $.each(board.field, function (lineindex, line) {
        $.each(line, function (rowindex, field) {
          console.log('fieldsizeincreateboard',fieldSize);
          var tileclass = "field";
          var qid = "";
          if (!board.field[lineindex][rowindex].exists) {
            return;
          }
          ...
          $("#" + pos).css({
            top: lineindex * fieldSize + topSpace,
            left: rowindex * fieldSize + leftSpace,
            width: fieldSize,
            height: fieldSize,
          });
          $("#" + pos).data("qid", qid);
      ...
      var topshift = 0;
      if (
        $(window).width() - board.maxWidth * fieldSize >
        $(window).height() - board.maxHeight * fieldSize
      ) {
      ...
      var tilesPerLine = Math.floor($(window).width() / fieldSize) - 3;
      console.log('tielsperline',tilesPerLine);
      var neededLines = Math.ceil(board.deck.length / tilesPerLine);
      if (
        $(window).height() >
        (neededLines + board.maxHeight + 1) * fieldSize
      ) {
        topshift = board.maxHeight;
        leftshift = 0;
      } else {
      ...
      if (articleleft > 0) {
        $("#article").css({
          left: (articleleft + 0.5) * fieldSize + leftSpace,
        });
      } else {
      ...
      $.each(board.deck, function (index, qid) {
        ...
        $("#" + qid).css({
          left:
            ((index % tilesPerLine) + leftshift) * fieldSize +
            leftSpace +
            wiggleSpace +
            Math.floor(Math.random() * wiggleSpace),
          top:
            (Math.floor(index / tilesPerLine) + topshift) * fieldSize +
            topSpace +
            wiggleSpace +
            Math.floor(Math.random() * wiggleSpace),
          width: fieldSize,
    ...
    var loadExtracts = function (toLoad, qids, lang, articles) {
      ...
      $.ajax({
        dataType: "jsonp",
        url:
          "https://en.wikipedia.org/w/api.php?action=query&format=json&prop=extracts&exsentences=3&exlimit=20&exintro=1&titles=" +
          loadNow.join("|"),
    ...
    //Function: Initialize the playing board
    var initBoard = function () {
      var boardParam = urlParam("board");
      if (boardParam != null && !validateBoard(boardParam)) {
        console.log(
          "Using default board as parameter board is invalid: " + boardParam,
        );
        boardParam = null;
      }
      if (boardParam == null) {
      ...
      board.field = $.map(lines, function (line) {
        return [
          $.map(line.substring(1).split("Q"), function (value) {
            var field = {};
      ...
      board.maxWidth = Math.max.apply(
        null,
        $.map(board.field, function (line) {
          return line.length;
        }),
      );
      console.log('boardmaxwidth',board.maxWidth);
      console.log('windowwidth',windowWidth);
      // Calculate maximum width based on field size
      var maxBoardWidth = board.maxWidth * fieldSize;

      // Adjust fieldSize if maxBoardWidth exceeds window width
      if (maxBoardWidth > (windowWidth-30)) {
        fieldSize = Math.floor(windowWidth / board.maxWidth) - 10; // Adjust field size
      }
      if (fieldSize < 80) {
        fieldSize = 80;
      }
      board.fieldSize = fieldSize;
    ...
    var showConnection = function (y1, x1, y2, x2, p, kb, language) {
      ...
      if (p != false) {
        ...
        elem = $(
          '<div class="connection fit">&nbsp;<span class="popup">' +
            proplabel +
            "</span></div>",
        );
    ...
    var loadProperties = function (entities, args) {
      ...
      loadEntities(properties, args.kb, loadArticles, {
        lang: args.language,
    ...
    var loadBoardEntities = function (kb, language, board, articles) {
      ...
      loadEntities(boardEntities, kb, loadProperties, {
        board: board,
        language: language,
        kb: kb,
        articles: articles,
    ...
    var getLabel = function (item, language) {
      ...
      if (
        ...
        typeof item.labels[language] == "undefined"
      ) {
        if (language == "en") {
          return item.id;
        } else {
    ...
    var getDescription = function (item, language) {
      if (typeof item == "undefined") return "";
      if (
        typeof item["descriptions"] == "undefined" ||
        typeof item.descriptions[language] == "undefined"
      ) {
        if (language == "en") {
          return "";
        } else {
    ...
    var setImage = function (item, tile) {
      ...
      $("#imageSourcesList").append(
        '<p><a href="https://commons.wikimedia.org/wiki/File:' +
          filename +
          '">' +
          filename +
          "</a></p>",
      ...
      $.getJSON(
        "https://commons.wikimedia.org/w/api.php?callback=?",
        {
          action: "query",
          formatversion: 2,
          prop: "imageinfo",
          ...
          if (board.field[y][x].exists) {
            ...
            if (!(f.solution in articles)) {
              articles[f.solution] =
                "<p><b>" + l + "</b>  <i>" + d + "</i></p>";
              if ($("#article").html() == "") {
                $("#article").html(articles[f.solution]);
              }
            }
            $(tile + " span").text(l);
        ...
        $.each(v, function (i, v) {
          ...
          if (v.mainsnak.datatype == "wikibase-item")
            if (v.mainsnak.snaktype == "value")
              if (v.mainsnak.datavalue.value.id == o) connection = p;
        });
      });
      return connection;
    };
    ...
    var solvableBoard = function (board, kb) {
      ...
      if (!allFit) {
        ...
        $("#gameTitle").after(
          '<p id="warn">Warning! This level might be unsolvable <a href="about.html#unsolvable">[see more]</a></p>',
        );
      }
    };
    createBoard(initLanguage(), initBoard());

    $("#imageSources").click(function () {
      $("#imageSourcesList").toggle();
    });
